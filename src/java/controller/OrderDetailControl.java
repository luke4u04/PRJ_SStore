...
public class OrderDetailControl extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        List<DetailProduct> listD = dao.getDetailProductByMaHD(mahd);
        Invoice invoice = dao.getInvoiceByMaHD(mahd);
        SimpleDateFormat dateFormat = new SimpleDateFormat("dd-MM-yyyy");

        Calendar c1 = Calendar.getInstance();
        c1.setTime(invoice.getNgayXuat());
        String dateEstimate = dateFormat.format(c1.getTime());
        String statusName = dao.getStatusName(sidInt);

        request.setAttribute("estimateDelivary", dateEstimate);

        request.setAttribute("listD", listD);
        request.setAttribute("invoice", invoice);
        request.setAttribute("status", statusName);
        request.setAttribute("l", local);
